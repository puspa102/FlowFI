...
function FeatureCard({ feature, index }: { feature: typeof features[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-3xl border ${feature.border} bg-white/[0.025] p-7 hover:bg-white/[0.04] transition-all duration-400 card-hover hover:shadow-2xl ${feature.glow} ${feature.span}`}
    >
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-lg font-bold text-white mb-2 group-hover:gradient-text-cyan transition-all duration-300">
          {feature.title}
        </h3>
        <p className="text-sm text-white/50 leading-relaxed">{feature.desc}</p>

        {/* Hover arrow */}
        <div className="mt-5 flex items-center gap-2 text-xs font-semibold text-white/30 group-hover:text-white/60 transition-colors duration-300">
          Learn more
...
export default function FeaturesSection() {
  ...
  return (
    <section className="relative bg-[#050d1f] py-28 overflow-hidden" id="features">
      {/* Ambient glow top */}
